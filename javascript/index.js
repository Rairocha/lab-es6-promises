...
.then((values) => values.forEach((v)=> document.querySelector("#brusselsSprouts").innerHTML +=`<li>${v}</li>`))
.catch((error) => console.log(error))
.finally(() => {
  document.querySelector("#brusselsSprouts").innerHTML +=`<li>Brussels Sprouts is ready!</li>`;
  document.querySelector("#brusselsSproutsImg").removeAttribute("hidden")})*/

Promise.all(Array.from(Array(brusselsSprouts.length).keys()).map((i)=>obtainInstruction("brusselsSprouts",i)))
.then((values) => values.forEach((v)=> document.querySelector("#brusselsSprouts").innerHTML +=`<li>${v}</li>`))
.catch((error) => console.log(error))
.finally(() => {
  document.querySelector("#brusselsSprouts").innerHTML +=`<li>Brussels Sprouts is ready!</li>`;
